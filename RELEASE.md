...
> **アカウント削除の動画を審査メモに入れる**（ガイドライン5.1.1(v)の指摘への対応）:
> 設定 → データ → 「アカウントとすべてのデータを削除」で完結するようにコード側は対応済み。
> Appleは**実機で撮った画面収録**を求めてくるので、次の3点が1本に入るように撮る。
>
> 1. デモアカウントでログインする（または新規にログインする）
> 2. 設定タブ → 「データ」の「アカウントとすべてのデータを削除」まで辿る
> 3. 確認ダイアログ →（本人確認のため再ログイン）→ 削除完了のトーストまで
>
> 撮った動画は App Store Connect の「App Review Information」の Notes 欄に添える。
> 削除後はログアウト状態・リンク0件に戻るので、そこまで映すと分かりやすい。
> Firestoreルールの変更は不要（`allow write` に delete が含まれる）。
>
> **写真ライブラリの権限が増えた**: 詳細画面の「保存」でサムネイル画像を
> 写真ライブラリへ入れるため、`NSPhotoLibraryAddUsageDescription`（追加のみ）を追加した。
> App Store Connectの審査メモに「保存したリンクの画像を写真に追加する機能」と書いておくと早い。
> 読み取り権限（`NSPhotoLibraryUsageDescription`）は要求していないので、
> 追加以外の写真APIを呼ばないよう注意する（呼ぶとiOSが強制終了させる）。
